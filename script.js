...
function renderFinal(){
  ...
  function drawBack(yO){
    ...
    ctx.setLineDash([]);
    // left
    ctx.font=`${W*.013}px "Special Elite",cursive`;ctx.fillStyle=ink.c;ctx.textAlign='left';
    ctx.fillText('THIS SPACE FOR WRITING MESSAGES',pad,yO+H*.14);
    ctx.font=`${W*.023}px "Special Elite",cursive`;
    (function(){
      const lh=W*.031;
      // count total lines
      const allLines=[];
      ...
      // vertical center within writing area (top H*.14, bottom H*.84, minus signature space)
      const areaTop=yO+H*.16;
      const areaBot=yO+H*.82;
      const areaH=areaBot-areaTop;
      const totalTextH=allLines.length*lh;
      const startY=areaTop+Math.max(0,(areaH-totalTextH)/2)+lh*.75;
      let cy=startY;
      msg.split('\n').forEach(para=>{
        if(!para.trim()){cy+=lh*.6;return;}
        const words=para.split(' ');let line='';
...
function go(n){
  ...
  if(n===1){
    // reset share/download buttons to default state
    const sb=document.getElementById('shareBtn');
    const db=document.getElementById('dlBtn');
    if(sb){sb.style.display='none';sb.textContent='↑ share postcard';}
    if(db){db.textContent='↓ download postcard';db.style.background='';db.style.color='';db.style.border='';db.style.boxShadow='';}
...
async function setupShareDownload(){
  const canvas = document.getElementById('finalC');
  const shareBtn = document.getElementById('shareBtn');
  const dlBtn = document.getElementById('dlBtn');

  // check Web Share API file support
  const testBlob = await getCanvasBlob(canvas);
  const testFile = new File([testBlob],'postcard.png',{type:'image/png'});
  const canShareFile = navigator.canShare && navigator.canShare({files:[testFile]});
  const canShareBasic = !!navigator.share;

  if(canShareFile){
    // full file sharing — show as hero
    ...
    // basic share — no file, just text + url
    shareBtn.style.display = 'block';
    shareBtn.textContent = '↑ share a link';
    dlBtn.textContent = '↓ download postcard';

    shareBtn.onclick = async () => {
      try {
        await navigator.share({
          title: 'A postcard for you',
          text: 'I made you a postcard ✦',
  ...
  // if no share support at all, shareBtn stays hidden, download is the only CTA
}
...
// ── Vertically center textarea text via padding trick ──
function centerTextarea(){
  const ta = document.getElementById('msgInput');
  if(!ta) return;
  ta.style.paddingTop = '0px';
  // force reflow
  const totalH = ta.scrollHeight;
  const visH = ta.clientHeight;
  if(totalH < visH){
    ta.style.paddingTop = Math.floor((visH - totalH)/2) + 'px';
  }
}
const msgTa = document.getElementById('msgInput');
if(msgTa){
  msgTa.addEventListener('input', centerTextarea);
  msgTa.addEventListener('focus', centerTextarea);
  msgTa.addEventListener('blur', centerTextarea);
  // also run when screen 3 is shown — patch go()
  const _go = go;
  go = function(n){
    _go(n);
    if(n===3) setTimeout(centerTextarea, 120);
  };
}